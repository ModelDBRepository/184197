'''
Current clamp demo using Allen Brain's model 472421285.
...
from neuron472421285 import Neuron472421285
...
def demo(iapp):
    ...
    h.celsius = 34.0
    cell = Neuron472421285(name='neuron')
    ...
    vbox.intercept(0)
    vbox.map('Allen Institute model 472421285; v includes junction potential', 10, 10, 600, 400)

    # procedure for doing and plotting each simulation
    def do_current_clamp_experiment(amp, color):   
        ic.amp = amp / 1000.
        
        h.finitialize(-91.8781604767)
        h.fcurrent()
